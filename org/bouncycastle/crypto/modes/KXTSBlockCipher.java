...
public class KXTSBlockCipher extends DefaultBufferedBlockCipher {
  ...
  public int processBytes(byte[] paramArrayOfbyte1, int paramInt1, int paramInt2, byte[] paramArrayOfbyte2, int paramInt3) {
    ...
    for (i = 0; i < paramInt2; i += this.blockSize)
      processBlocks(paramArrayOfbyte1, paramInt1 + i, paramArrayOfbyte2, paramInt3 + i); 
    return paramInt2;
  }
  
  private void processBlocks(byte[] paramArrayOfbyte1, int paramInt1, byte[] paramArrayOfbyte2, int paramInt2) {
    if (this.counter == -1)
...
/* Location:              C:\Program Files\BurpSuitePro\burpsuite_pro.jar!\org\bouncycastle\crypto\modes\KXTSBlockCipher.class
