...
public class GCMSIVBlockCipher implements AEADBlockCipher {
  ...
  public GCMSIVBlockCipher(BlockCipher paramBlockCipher) {
    this(paramBlockCipher, (GCMMultiplier)new Tables4kGCMMultiplier());
  }
  ...
  private void deriveKeys(KeyParameter paramKeyParameter) {
    ...
    byte[] arrayOfByte2 = new byte[16];
    byte[] arrayOfByte3 = new byte[16];
    byte[] arrayOfByte4 = new byte[paramKeyParameter.getKeyLength()];
    System.arraycopy(this.theNonce, 0, arrayOfByte1, 4, 12);
    this.theCipher.init(true, (CipherParameters)paramKeyParameter);
    boolean bool = false;
    this.theCipher.processBlock(arrayOfByte1, 0, arrayOfByte2, 0);
    ...
    this.theCipher.init(true, (CipherParameters)new KeyParameter(arrayOfByte4));
...
/* Location:              C:\Program Files\BurpSuitePro\burpsuite_pro.jar!\org\bouncycastle\crypto\modes\GCMSIVBlockCipher.class
