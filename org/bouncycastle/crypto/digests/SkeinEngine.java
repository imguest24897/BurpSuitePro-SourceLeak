...
public class SkeinEngine implements Memoable {
  ...
  private static Integer variantIdentifier(int paramInt1, int paramInt2) {
    return Integers.valueOf(paramInt2 << 16 | paramInt1);
  }
  ...
  private void output(long paramLong, byte[] paramArrayOfbyte, int paramInt1, int paramInt2) {
    ...
    Pack.longToLittleEndian(paramLong, arrayOfByte, 0);
    ...
    for (byte b = 0; b < i; b++) {
      ...
      if (j == 8) {
        Pack.longToLittleEndian(arrayOfLong[b], paramArrayOfbyte, paramInt1 + b * 8);
      } else {
        Pack.longToLittleEndian(arrayOfLong[b], arrayOfByte, 0);
  ...
  private static class Configuration {
    ...
    public Configuration(long param1Long) {
      ...
      this.bytes[4] = 1;
      this.bytes[5] = 0;
      Pack.longToLittleEndian(param1Long, this.bytes, 8);
    }
  ...
  private class UBI {
    ...
    private void processBlock(long[] param1ArrayOflong) {
      ...
      for (byte b = 0; b < param1ArrayOflong.length; b++)
...
/* Location:              C:\Program Files\BurpSuitePro\burpsuite_pro.jar!\org\bouncycastle\crypto\digests\SkeinEngine.class
