  ...
  protected Key engineDoPhase(Key paramKey, boolean paramBoolean) throws InvalidKeyException, IllegalStateException {
    ...
    try {
      if (this.agreement instanceof BasicAgreement) {
        this.result = bigIntToBytes(((BasicAgreement)this.agreement).calculateAgreement((CipherParameters)asymmetricKeyParameter));
      } else {
        this.result = ((ECDHCUnifiedAgreement)this.agreement).calculateAgreement((CipherParameters)asymmetricKeyParameter);
      } 
  ...
  protected void doInitFromKey(Key paramKey, AlgorithmParameterSpec paramAlgorithmParameterSpec, SecureRandom paramSecureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
    ...
    if (this.agreement instanceof ECMQVBasicAgreement) {
      ...
      if (paramKey instanceof MQVPrivateKey) {
        ...
        eCPrivateKeyParameters1 = (ECPrivateKeyParameters)ECUtils.generatePrivateKeyParameter(mQVPrivateKey.getStaticPrivateKey());
        eCPrivateKeyParameters2 = (ECPrivateKeyParameters)ECUtils.generatePrivateKeyParameter(mQVPrivateKey.getEphemeralPrivateKey());
        ...
        eCPrivateKeyParameters1 = (ECPrivateKeyParameters)ECUtils.generatePrivateKeyParameter((PrivateKey)paramKey);
        eCPrivateKeyParameters2 = (ECPrivateKeyParameters)ECUtils.generatePrivateKeyParameter(mQVParameterSpec.getEphemeralPrivateKey());
        eCPublicKeyParameters = null;
      ...
      ((ECMQVBasicAgreement)this.agreement).init((CipherParameters)mQVPrivateParameters);
      ...
      ECPrivateKeyParameters eCPrivateKeyParameters1 = (ECPrivateKeyParameters)ECUtils.generatePrivateKeyParameter((PrivateKey)paramKey);
      ECPrivateKeyParameters eCPrivateKeyParameters2 = (ECPrivateKeyParameters)ECUtils.generatePrivateKeyParameter(dHUParameterSpec.getEphemeralPrivateKey());
      ...
      this.parameters = eCPrivateKeyParameters1.getParameters();
      ((ECDHCUnifiedAgreement)this.agreement).init((CipherParameters)eCDHUPrivateParameters);
    } else {
      ...
      ECPrivateKeyParameters eCPrivateKeyParameters = (ECPrivateKeyParameters)ECUtils.generatePrivateKeyParameter((PrivateKey)paramKey);
      ...
      ((BasicAgreement)this.agreement).init((CipherParameters)eCPrivateKeyParameters);
    } 
  }
...
/* Location:              C:\Program Files\BurpSuitePro\burpsuite_pro.jar!\org\bouncycastle\jcajce\provider\asymmetric\ec\KeyAgreementSpi.class
