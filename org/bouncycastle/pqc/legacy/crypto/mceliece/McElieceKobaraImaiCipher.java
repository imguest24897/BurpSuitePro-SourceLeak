...
public class McElieceKobaraImaiCipher implements MessageEncryptor {
  ...
  public byte[] messageDecrypt(byte[] paramArrayOfbyte) throws InvalidCipherTextException {
    ...
    int k = this.k >> 3;
    int m = IntegerFunctions.binomial(this.n, this.t).bitLength() - 1 >> 3;
    int n = paramArrayOfbyte.length - i;
    if (n > 0) {
      byte[][] arrayOfByte = ByteUtils.split(paramArrayOfbyte, n);
    ...
    if (arrayOfByte4.length < m) {
      byte[] arrayOfByte = new byte[m];
      System.arraycopy(arrayOfByte4, 0, arrayOfByte, m - arrayOfByte4.length, arrayOfByte4.length);
    ...
    for (int i2 = j - 1; i2 >= 0; i2--)
      arrayOfByte9[i2] = (byte)(arrayOfByte9[i2] ^ arrayOfByte7[i2]); 
    DigestRandomGenerator digestRandomGenerator = new DigestRandomGenerator((Digest)new SHA1Digest());
    digestRandomGenerator.addSeedMaterial(arrayOfByte9);
    byte[] arrayOfByte10 = new byte[i1];
    digestRandomGenerator.nextBytes(arrayOfByte10);
    for (int i3 = i1 - 1; i3 >= 0; i3--)
      arrayOfByte10[i3] = (byte)(arrayOfByte10[i3] ^ arrayOfByte8[i3]); 
    if (arrayOfByte10.length < i1)
      throw new InvalidCipherTextException("Bad Padding: invalid ciphertext"); 
    byte[][] arrayOfByte11 = ByteUtils.split(arrayOfByte10, i1 - PUBLIC_CONSTANT.length);
...
/* Location:              C:\Program Files\BurpSuitePro\burpsuite_pro.jar!\org\bouncycastle\pqc\legacy\crypto\mceliece\McElieceKobaraImaiCipher.class
