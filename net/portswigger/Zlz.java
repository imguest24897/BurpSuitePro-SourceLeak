package net.portswigger;

import java.util.Iterator;

public class Zlz {
  private static final String[] a;
  
  private static final String[] b;
  
  public static int ZR(String paramString1, String paramString2) {
    // Byte code:
    //   0: invokestatic ZZ : ()I
    //   3: istore_2
    //   4: aload_0
    //   5: invokestatic ZC : (Ljava/lang/String;)Z
    //   8: ifne -> 25
    //   11: aload_1
    //   12: invokestatic ZC : (Ljava/lang/String;)Z
    //   15: ifeq -> 31
    //   18: goto -> 25
    //   21: invokestatic a : (Ljava/lang/IllegalArgumentException;)Ljava/lang/IllegalArgumentException;
    //   24: athrow
    //   25: iconst_0
    //   26: istore_3
    //   27: iload_2
    //   28: ifne -> 66
    //   31: iconst_0
    //   32: istore_3
    //   33: iconst_0
    //   34: istore #4
    //   36: aload_0
    //   37: aload_1
    //   38: iload #4
    //   40: invokevirtual indexOf : (Ljava/lang/String;I)I
    //   43: dup
    //   44: istore #4
    //   46: iconst_m1
    //   47: if_icmpeq -> 66
    //   50: iinc #3, 1
    //   53: iload #4
    //   55: aload_1
    //   56: invokevirtual length : ()I
    //   59: iadd
    //   60: istore #4
    //   62: iload_2
    //   63: ifne -> 36
    //   66: iload_3
    //   67: ireturn
    // Exception table:
    //   from	to	target	type
    //   4	18	21	java/lang/IllegalArgumentException
  }
  
  public static String Ze(String paramString1, String paramString2) {
    String str;
    try {
      if (ZC(paramString1) || paramString2 == null) {
        str = paramString1;
      } else if (paramString2.isEmpty()) {
        str = "";
      } else {
        int i = paramString1.indexOf(paramString2);
        if (i == -1) {
          str = paramString1;
        } else {
          str = paramString1.substring(0, i);
        } 
      } 
    } catch (IllegalArgumentException illegalArgumentException) {
      throw a(null);
    } 
    return str;
  }
  
  public static boolean ZC(String paramString) {
    try {
      if (paramString != null) {
        try {
          if (paramString.isEmpty());
        } catch (IllegalArgumentException illegalArgumentException) {
          throw a(null);
        } 
        return false;
      } 
    } catch (IllegalArgumentException illegalArgumentException) {
      throw a(null);
    } 
  }
  
  public static boolean Zr(String paramString) {
    try {
    
    } catch (IllegalArgumentException illegalArgumentException) {
      throw a(null);
    } 
    return !ZC(paramString);
  }
  
  public static boolean ZX(String paramString) {
    try {
      if (paramString != null) {
        try {
          if (paramString.trim().isEmpty());
        } catch (IllegalArgumentException illegalArgumentException) {
          throw a(null);
        } 
        return false;
      } 
    } catch (IllegalArgumentException illegalArgumentException) {
      throw a(null);
    } 
  }
  
  public static boolean Zu(String paramString) {
    try {
    
    } catch (IllegalArgumentException illegalArgumentException) {
      throw a(null);
    } 
    return !ZX(paramString);
  }
  
  public static String ZE(Iterator<?> paramIterator, String paramString) {
    // Byte code:
    //   0: invokestatic ZK : ()I
    //   3: istore_2
    //   4: aload_0
    //   5: ifnonnull -> 14
    //   8: aconst_null
    //   9: astore_3
    //   10: iload_2
    //   11: ifeq -> 158
    //   14: aload_0
    //   15: invokeinterface hasNext : ()Z
    //   20: ifne -> 37
    //   23: goto -> 30
    //   26: invokestatic a : (Ljava/lang/IllegalArgumentException;)Ljava/lang/IllegalArgumentException;
    //   29: athrow
    //   30: ldc ''
    //   32: astore_3
    //   33: iload_2
    //   34: ifeq -> 158
    //   37: aload_0
    //   38: invokeinterface next : ()Ljava/lang/Object;
    //   43: astore #4
    //   45: aload_0
    //   46: invokeinterface hasNext : ()Z
    //   51: ifne -> 64
    //   54: aload #4
    //   56: invokestatic valueOf : (Ljava/lang/Object;)Ljava/lang/String;
    //   59: astore_3
    //   60: iload_2
    //   61: ifeq -> 158
    //   64: new java/lang/StringBuilder
    //   67: dup
    //   68: invokespecial <init> : ()V
    //   71: astore #5
    //   73: aload #4
    //   75: ifnull -> 93
    //   78: aload #5
    //   80: aload #4
    //   82: invokevirtual append : (Ljava/lang/Object;)Ljava/lang/StringBuilder;
    //   85: pop
    //   86: goto -> 93
    //   89: invokestatic a : (Ljava/lang/IllegalArgumentException;)Ljava/lang/IllegalArgumentException;
    //   92: athrow
    //   93: aload_0
    //   94: invokeinterface hasNext : ()Z
    //   99: ifeq -> 152
    //   102: aload_1
    //   103: ifnull -> 120
    //   106: aload #5
    //   108: aload_1
    //   109: invokevirtual append : (Ljava/lang/String;)Ljava/lang/StringBuilder;
    //   112: pop
    //   113: goto -> 120
    //   116: invokestatic a : (Ljava/lang/IllegalArgumentException;)Ljava/lang/IllegalArgumentException;
    //   119: athrow
    //   120: aload_0
    //   121: invokeinterface next : ()Ljava/lang/Object;
    //   126: astore #6
    //   128: aload #6
    //   130: ifnull -> 148
    //   133: aload #5
    //   135: aload #6
    //   137: invokevirtual append : (Ljava/lang/Object;)Ljava/lang/StringBuilder;
    //   140: pop
    //   141: goto -> 148
    //   144: invokestatic a : (Ljava/lang/IllegalArgumentException;)Ljava/lang/IllegalArgumentException;
    //   147: athrow
    //   148: iload_2
    //   149: ifeq -> 93
    //   152: aload #5
    //   154: invokevirtual toString : ()Ljava/lang/String;
    //   157: astore_3
    //   158: aload_3
    //   159: areturn
    // Exception table:
    //   from	to	target	type
    //   10	23	26	java/lang/IllegalArgumentException
    //   73	86	89	java/lang/IllegalArgumentException
    //   102	113	116	java/lang/IllegalArgumentException
    //   128	141	144	java/lang/IllegalArgumentException
  }
  
  public static String Zh(Iterable<?> paramIterable, String paramString) {
    int i = Zkb.ZK();
    if (paramIterable == null) {
      String str = null;
      return (i != 0) ? ZE(paramIterable.iterator(), paramString) : str;
    } 
    return ZE(paramIterable.iterator(), paramString);
  }
  
  public static String ZL(String paramString, int paramInt1, int paramInt2) {
    try {
      if (paramString == null)
        return null; 
    } catch (IllegalArgumentException illegalArgumentException) {
      throw a(null);
    } 
    try {
      if (paramString.length() <= paramInt1)
        return paramString; 
    } catch (IllegalArgumentException illegalArgumentException) {
      throw a(null);
    } 
    return paramString.substring(0, paramInt2) + paramString.substring(0, paramInt2);
  }
  
  public static String ZB(String paramString, int paramInt) {
    try {
      if (paramInt < 5)
        throw new IllegalArgumentException(); 
    } catch (IllegalArgumentException illegalArgumentException) {
      throw a(null);
    } 
    try {
      if (paramString.length() <= paramInt)
        return paramString; 
    } catch (IllegalArgumentException illegalArgumentException) {
      throw a(null);
    } 
    int i = (int)Math.ceil(paramInt / 2.0D) - 2;
    int j = (int)Math.floor(paramInt / 2.0D) - 1;
    return paramString.substring(0, i) + paramString.substring(0, i) + a(20592, -27604);
  }
  
  private static IllegalArgumentException a(IllegalArgumentException paramIllegalArgumentException) {
    return paramIllegalArgumentException;
  }
  
  static {
    // Byte code:
    //   0: iconst_2
    //   1: anewarray java/lang/String
    //   4: astore #5
    //   6: iconst_0
    //   7: istore_3
    //   8: ldc '¿î\\tÎÁè'
    //   10: dup
    //   11: astore_2
    //   12: invokevirtual length : ()I
    //   15: istore #4
    //   17: iconst_3
    //   18: istore_1
    //   19: iconst_m1
    //   20: istore_0
    //   21: bipush #106
    //   23: iinc #0, 1
    //   26: aload_2
    //   27: iload_0
    //   28: dup
    //   29: iload_1
    //   30: iadd
    //   31: invokevirtual substring : (II)Ljava/lang/String;
    //   34: iconst_m1
    //   35: goto -> 81
    //   38: aload #5
    //   40: swap
    //   41: iload_3
    //   42: iinc #3, 1
    //   45: swap
    //   46: aastore
    //   47: iload_0
    //   48: iload_1
    //   49: iadd
    //   50: dup
    //   51: istore_0
    //   52: iload #4
    //   54: if_icmpge -> 66
    //   57: aload_2
    //   58: iload_0
    //   59: invokevirtual charAt : (I)C
    //   62: istore_1
    //   63: goto -> 21
    //   66: aload #5
    //   68: putstatic net/portswigger/Zlz.a : [Ljava/lang/String;
    //   71: iconst_2
    //   72: anewarray java/lang/String
    //   75: putstatic net/portswigger/Zlz.b : [Ljava/lang/String;
    //   78: goto -> 227
    //   81: dup_x2
    //   82: pop
    //   83: invokevirtual toCharArray : ()[C
    //   86: dup_x1
    //   87: arraylength
    //   88: dup_x2
    //   89: pop
    //   90: iconst_0
    //   91: istore #6
    //   93: dup2_x1
    //   94: pop2
    //   95: dup_x2
    //   96: iconst_1
    //   97: if_icmpgt -> 200
    //   100: dup2
    //   101: swap
    //   102: iload #6
    //   104: dup2_x1
    //   105: caload
    //   106: swap
    //   107: iload #6
    //   109: bipush #7
    //   111: irem
    //   112: tableswitch default -> 182, 0 -> 152, 1 -> 157, 2 -> 162, 3 -> 167, 4 -> 172, 5 -> 177
    //   152: bipush #125
    //   154: goto -> 184
    //   157: bipush #100
    //   159: goto -> 184
    //   162: bipush #98
    //   164: goto -> 184
    //   167: bipush #95
    //   169: goto -> 184
    //   172: bipush #71
    //   174: goto -> 184
    //   177: bipush #125
    //   179: goto -> 184
    //   182: bipush #13
    //   184: ixor
    //   185: ixor
    //   186: i2c
    //   187: castore
    //   188: iinc #6, 1
    //   191: dup
    //   192: ifne -> 200
    //   195: dup2
    //   196: dup_x1
    //   197: goto -> 104
    //   200: dup2_x1
    //   201: pop2
    //   202: dup_x2
    //   203: iload #6
    //   205: if_icmpgt -> 100
    //   208: pop
    //   209: new java/lang/String
    //   212: dup_x1
    //   213: swap
    //   214: invokespecial <init> : ([C)V
    //   217: invokevirtual intern : ()Ljava/lang/String;
    //   220: swap
    //   221: pop
    //   222: swap
    //   223: pop
    //   224: goto -> 38
    //   227: return
  }
  
  private static String a(int paramInt1, int paramInt2) {
    int i = (paramInt1 ^ 0x5070) & 0xFFFF;
    if (b[i] == null) {
      char[] arrayOfChar = a[i].toCharArray();
      switch (arrayOfChar[0] & 0xFF) {
        case 0:
        
        case 1:
        
        case 2:
        
        case 3:
        
        case 4:
        
        case 5:
        
        case 6:
        
        case 7:
        
        case 8:
        
        case 9:
        
        case 10:
        
        case 11:
        
        case 12:
        
        case 13:
        
        case 14:
        
        case 15:
        
        case 16:
        
        case 17:
        
        case 18:
        
        case 19:
        
        case 20:
        
        case 21:
        
        case 22:
        
        case 23:
        
        case 24:
        
        case 25:
        
        case 26:
        
        case 27:
        
        case 28:
        
        case 29:
        
        case 30:
        
        case 31:
        
        case 32:
        
        case 33:
        
        case 34:
        
        case 35:
        
        case 36:
        
        case 37:
        
        case 38:
        
        case 39:
        
        case 40:
        
        case 41:
        
        case 42:
        
        case 43:
        
        case 44:
        
        case 45:
        
        case 46:
        
        case 47:
        
        case 48:
        
        case 49:
        
        case 50:
        
        case 51:
        
        case 52:
        
        case 53:
        
        case 54:
        
        case 55:
        
        case 56:
        
        case 57:
        
        case 58:
        
        case 59:
        
        case 60:
        
        case 61:
        
        case 62:
        
        case 63:
        
        case 64:
        
        case 65:
        
        case 66:
        
        case 67:
        
        case 68:
        
        case 69:
        
        case 70:
        
        case 71:
        
        case 72:
        
        case 73:
        
        case 74:
        
        case 75:
        
        case 76:
        
        case 77:
        
        case 78:
        
        case 79:
        
        case 80:
        
        case 81:
        
        case 82:
        
        case 83:
        
        case 84:
        
        case 85:
        
        case 86:
        
        case 87:
        
        case 88:
        
        case 89:
        
        case 90:
        
        case 91:
        
        case 92:
        
        case 93:
        
        case 94:
        
        case 95:
        
        case 96:
        
        case 97:
        
        case 98:
        
        case 99:
        
        case 100:
        
        case 101:
        
        case 102:
        
        case 103:
        
        case 104:
        
        case 105:
        
        case 106:
        
        case 107:
        
        case 108:
        
        case 109:
        
        case 110:
        
        case 111:
        
        case 112:
        
        case 113:
        
        case 114:
        
        case 115:
        
        case 116:
        
        case 117:
        
        case 118:
        
        case 119:
        
        case 120:
        
        case 121:
        
        case 122:
        
        case 123:
        
        case 124:
        
        case 125:
        
        case 126:
        
        case 127:
        
        case 128:
        
        case 129:
        
        case 130:
        
        case 131:
        
        case 132:
        
        case 133:
        
        case 134:
        
        case 135:
        
        case 136:
        
        case 137:
        
        case 138:
        
        case 139:
        
        case 140:
        
        case 141:
        
        case 142:
        
        case 143:
        
        case 144:
        
        case 145:
        
        case 146:
        
        case 147:
        
        case 148:
        
        case 149:
        
        case 150:
        
        case 151:
        
        case 152:
        
        case 153:
        
        case 154:
        
        case 155:
        
        case 156:
        
        case 157:
        
        case 158:
        
        case 159:
        
        case 160:
        
        case 161:
        
        case 162:
        
        case 163:
        
        case 164:
        
        case 165:
        
        case 166:
        
        case 167:
        
        case 168:
        
        case 169:
        
        case 170:
        
        case 171:
        
        case 172:
        
        case 173:
        
        case 174:
        
        case 175:
        
        case 176:
        
        case 177:
        
        case 178:
        
        case 179:
        
        case 180:
        
        case 181:
        
        case 182:
        
        case 183:
        
        case 184:
        
        case 185:
        
        case 186:
        
        case 187:
        
        case 188:
        
        case 189:
        
        case 190:
        
        case 191:
        
        case 192:
        
        case 193:
        
        case 194:
        
        case 195:
        
        case 196:
        
        case 197:
        
        case 198:
        
        case 199:
        
        case 200:
        
        case 201:
        
        case 202:
        
        case 203:
        
        case 204:
        
        case 205:
        
        case 206:
        
        case 207:
        
        case 208:
        
        case 209:
        
        case 210:
        
        case 211:
        
        case 212:
        
        case 213:
        
        case 214:
        
        case 215:
        
        case 216:
        
        case 217:
        
        case 218:
        
        case 219:
        
        case 220:
        
        case 221:
        
        case 222:
        
        case 223:
        
        case 224:
        
        case 225:
        
        case 226:
        
        case 227:
        
        case 228:
        
        case 229:
        
        case 230:
        
        case 231:
        
        case 232:
        
        case 233:
        
        case 234:
        
        case 235:
        
        case 236:
        
        case 237:
        
        case 238:
        
        case 239:
        
        case 240:
        
        case 241:
        
        case 242:
        
        case 243:
        
        case 244:
        
        case 245:
        
        case 246:
        
        case 247:
        
        case 248:
        
        case 249:
        
        case 250:
        
        case 251:
        
        case 252:
        
        case 253:
        
        case 254:
        
        default:
          break;
      } 
      char c = '';
      int j = (paramInt2 & 0xFF) - c;
      if (j < 0)
        j += 256; 
      int k = ((paramInt2 & 0xFFFF) >>> 8) - c;
      if (k < 0)
        k += 256; 
      for (byte b = 0; b < arrayOfChar.length; b++) {
        int m = b % 2;
        if (m == 0) {
          arrayOfChar[b] = (char)(arrayOfChar[b] ^ j);
          j = ((j >>> 3 | j << 5) ^ arrayOfChar[b]) & 0xFF;
        } else {
          arrayOfChar[b] = (char)(arrayOfChar[b] ^ k);
          k = ((k >>> 3 | k << 5) ^ arrayOfChar[b]) & 0xFF;
        } 
      } 
      b[i] = (new String(arrayOfChar)).intern();
    } 
    return b[i];
  }
}


/* Location:              C:\Program Files\BurpSuitePro\burpsuite_pro.jar!\net\portswigger\Zlz.class
 * Java compiler version: 21 (65.0)
 * JD-Core Version:       1.1.3
 */